...
usage() {
  cat <<'USAGE'
Usage: scripts/dev-backend-stop.sh [--port PORT]

Stops the local backend process started by scripts/dev-backend-start.sh.
USAGE
}
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null; then
    ...
    echo "backend has stale PID ${pid:-<empty>}"
  fi
fi
...
echo "Failed to stop backend within 30 seconds: PIDs $(echo "$unique_pids" | tr '\n' ' ')" >&2
exit 1
